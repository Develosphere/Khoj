...
The FastAPI backend now includes Supabase JWT authentication.
...
BACKEND_CORS_METHODS=["GET","POST","OPTIONS","PUT","DELETE"]
...
Returns the authenticated user's Supabase ID, email, and metadata. Missing, malformed, expired, or invalid bearer tokens return `401 Unauthorized`.

## Backend tests

The authentication endpoint includes tests for unauthenticated access and mocked valid Supabase JWT access:
...
## Supabase MFA endpoints

Authenticated users can manage Supabase TOTP MFA factors through the backend auth router:
...
- `enroll` starts TOTP enrollment and returns a factor ID plus QR code URL.
- `challenge` creates a challenge for an enrolled factor.
- `verify` verifies a 6-digit TOTP code and returns AAL/session information.
- `unenroll` removes an MFA factor.
